...
3. While navigating a list and you need access to access both the current and next object, use index based navigation and don't use for loop.
'''
class Solution:
...
'''
Time Complexity: O(NlogN){sorting the intervals} + O(N^2){del will shift the elements}
Space Complexity: O(N){for sorted intervals} 
'''